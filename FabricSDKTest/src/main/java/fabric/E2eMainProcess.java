...
public class E2eMainProcess {
    ...
    private String channelName, chaincodeName;
    //private int
    ...
    public void run(String cln, String ccn){
        try {
            channelName = cln;
            chaincodeName = ccn;
    ...
    private void runFabricTest() throws Exception {
        ...
        runChannel(false, sampleOrg);
       // setTest(new String[]{"aaaa", "aasdasdzxcxzcsdad"});
        //System.out.println(queryTest("aaaa"));
    }
...
//          orderers.remove(anOrderer);
        ...
        // ChannelConfiguration channelConfiguration = new ChannelConfiguration(new File(this.getClass().getResource("/component/channel-artifacts/certchannel.tx").getPath()));
       // Channel mychannel = client.newChannel("certchannel", anOrderer, channelConfiguration, client.getChannelConfigurationSignature(channelConfiguration, peerAdmin));
    ...
    private void runChannel(boolean isInstallChaincode, SampleOrg sampleOrg) {
        ...
        try{
            ...
            if (isInstallChaincode){
                ...
                responses.clear();
                out("asdasd" + channel.getOrderers());
                initChaincode();
            }
          //  initChaincode();
            successful.clear();
...
//            }
            ...
            out(E2eMainProcess.class.getResource(privateKeyFile).getPath());
    ...
    public String[] setTest(String[] str){
        ...
        try {
           ...
           // chaincodeEndorsementPolicy.fromYamlFile(new File("src/main/resources/component/endorsement_policy.yaml"));
            ...
            result_set[0] += "sending transactionProposal to all peers with new key/value : " + str[0] + " / " + str[1] + "\n";
          ...
          //  System.out.println("背书发送前系统时间: "+ t1 + "  \n完成后系统时间: " + t2 + "  \n时间间隔为: " + (t2 - t1) +"ms");
           result_set[1] = t2 - t1 + "";
            for (ProposalResponse response : transactionPropResp) {
                if (response.getStatus() == ProposalResponse.Status.SUCCESS) {
                    out("Successful transaction proposal response Txid: %s from peer %s", response.getTransactionID(), response.getPeer().getName());
                    result_set[0] += "Successful transaction proposal response Txid: " + response.getTransactionID() + " from peer " + response.getPeer().getName() + "\n";
                    successful.add(response);
                } else {
                    failed.add(response);
                }
            ...
            result_set[0] += "Received " + transactionPropResp.size() + "transaction proposal responses. Successful+verified: "
            ...
            if (failed.size() > 0) {
                ...
                result_set[0] += "Not enough endorsers for invoke(set + " + str[0] + ", " + str[1] + "):" + failed.size() + " endorser error: " +
                        firstTransactionProposalResponse.getMessage() +
                        ". Was verified: " + firstTransactionProposalResponse.isVerified() + "\n";
            }
            ...
            result_set[0] += "Successfully received transaction proposal responses.\n";
            //  System.exit(10);
            ...
            result_set[0] += "Sending chaincode transaction to orderer.\n";
            ...
            t1 = System.currentTimeMillis();
              channel.sendTransaction(successful,channel.getOrderers()).get(30000, TimeUnit.SECONDS);
            //  System.out.println("end"+System.currentTimeMillis());
            t2 = System.currentTimeMillis();
            System.out.println("交易提交时系统时间: " + t1 + "   \n交易成块上链完成时间: "+ t2 + " \n时间间隔为: "+(t2-t1) + "ms");
            result_set[2] = t2 - t1 + "";
//            channel.sendTransaction(successful, createTransactionOptions() //Basically the default options but shows it's usage.
//                    .userContext(client.getUserContext()) //could be a different user context. this is the default.
//                    .shuffleOrders(false) // don't shuffle any orderers the default is true.
