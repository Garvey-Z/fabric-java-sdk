...
import test.hyperledger.fabric.sdk.TestConfigHelper;
import test.hyperledger.fabric.sdk.testutils.TestConfig;
...
import static test.hyperledger.fabric.sdk.testutils.TestUtils.resetConfig;
import static test.hyperledger.fabric.sdk.testutils.TestUtils.testRemovingAddingPeersOrderers;
...
public class E2eText  {
    ...
    private Vector<ChaincodeEventCapture> chaincodeEvents = new Vector<>();
           ...
           //     Collection<ProposalResponse> successful = new LinkedList<>();
             //   Collection<ProposalResponse> failed = new LinkedList<>();
                ...
                // Register a chaincode event listener that will trigger for any chaincode id and only for EXPECTED_EVENT_NAME event.
                 ...
                 chaincodeEventListenerHandle = fooChannel.registerChaincodeEventListener(Pattern.compile(".*"),
...
//            )transaction.
            completableFuture.thenApply(transactionEvent -> {
                ...
                try {
                    ...
                    set_result += "sending transactionProposal to all peers with new key/value : " + str[0] + " / " + str[1] + "\n";
                    ...
                    for (ProposalResponse response : transactionPropResp) {
                        if (response.getStatus() == ProposalResponse.Status.SUCCESS) {
                            ...
                            set_result += "Successful transaction proposal response Txid: " + response.getTransactionID() + " from peer " + response.getPeer().getName() + "\n";
                            successful.add(response);
                        } else {
                            failed.add(response);
                        }
                    ...
                    set_result += "Received " + transactionPropResp.size() + "transaction proposal responses. Successful+verified: "
                    ...
                    if (failed.size() > 0) {
                        ...
                        set_result += "Not enough endorsers for invoke(set + " + str[0] + ", " + str[1] + "):" + failed.size() + " endorser error: " +
                                firstTransactionProposalResponse.getMessage() +
                                ". Was verified: " + firstTransactionProposalResponse.isVerified() + "\n";
                    }
                    ...
                    set_result += "Successfully received transaction proposal responses.\n";
                    //  System.exit(10);
                    ...
                    set_result += "Sending chaincode transaction to orderer.\n";
            ...
            out("Channel info for : " + fooChannel.getName());
            out("Channel height: " + channelInfo.getHeight());
            String chainCurrentHash = Hex.encodeHexString(channelInfo.getCurrentBlockHash());
            String chainPreviousHash = Hex.encodeHexString(channelInfo.getPreviousBlockHash());
            out("Chain current block hash: " + chainCurrentHash);
            out("Chain previous block hash: " + chainPreviousHash);

            set_result += "#####通道区块信息\n";
            set_result += "Channel info for: " + fooChannel.getName() + "\n";
            set_result += "Channel height: " + channelInfo.getHeight() + "\n";
            set_result += "Chain current block hash " + chainCurrentHash + "\n";
            set_result += "Chain previous block hash " + chainPreviousHash + "\n";
            ...
            //
            FileOutputStream fs = new FileOutputStream(new File("/Users/apple/Desktop/e2e.txt"));
//            PrintStream ps = new PrintStream(fs);
//            List<ByteString> list = returnedBlock.getBlock().getMetadata().getMetadataList();
//            for(ByteString ls : list) {
//                //out(ls.toString());
//              //  ps.println(ls.toString(UTF_8));
//            }
//            ps.close();
//            out("12312qwee");
//            for(ByteString ls : list)
//                System.out.println(ls.toString(ISO_8859_1));
//
//            for(ByteString ls : list)
//                System.out.println(new String(ls.toByteArray(), UTF_8));
            ...
            out(testTxID);
            out(txInfo.getEnvelope().getPayload().toString());
            out(txInfo.getEnvelope().getSignature().toString());
           // out(txInfo.getProcessedTransaction());
            ...
            if (chaincodeEventListenerHandle != null) {
                ...
                final int numberEventsExpected = fooChannel.getEventHubs().size() +
                        fooChannel.getPeers(EnumSet.of(PeerRole.EVENT_SOURCE)).size();
        ...
        set_result += "#####\n";
        set_result += "key : " + str[0] + "\n";
        set_result += "value : " + str[1] + "\n";
        set_result += "set successful\n";
    ...
    public String QueryBlockByNumber(long num){

        String result_block = "";
        try {
            BlockInfo blockInfo = fooChannel.queryBlockByNumber(num);
            result_block += "ChannelID: " + blockInfo.getChannelId() + "\n";
            result_block += "BlockNumber: " + blockInfo.getBlockNumber() + "\n";
            result_block += "PreviousHash: " + Hex.encodeHexString(blockInfo.getPreviousHash()) + "\n";
            result_block += "DataHash: " + Hex.encodeHexString(blockInfo.getDataHash()) + "\n";
            result_block += "TransactionCount: " + blockInfo.getTransactionCount() + "\n";
            return result_block;
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    public static String bytesToString(byte[] input, String charSet) {
        ...
        ByteBuffer buffer = ByteBuffer.allocate(input.length + 10);
        buffer.put(input);
        buffer.flip();
